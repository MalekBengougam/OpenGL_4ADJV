...
struct Application
{
	...
	void Render(const uint32_t width, const uint32_t height)
	{
		...
		// on sait ici quels sont les attributs que l'on doit assigner dans le shader
		...
		// on doit forcement respecifier toutes les definitions d'attributs car elles sont dependantes
		// du VBO actuellement actif ... dans le prochain exemple les VAO nous aideront a reduire cette complexite
		...
		// notez le "true" indiquant que les couleurs [0..255] doivent etre converti en float [0..1] dans le vertex shader
...
int main(void)
{
	...
	window = glfwCreateWindow(960, 720, "OBJ Viewer", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
